...
/**
 * 全屏观看直播页面的聊天的Fragment
 */
public class ChatLiveFragment extends Fragment implements ChatContract.ChatView {
...
//    EditText editContent;
    ...
    TextView text_chat_content,tv_hide;
    ...
    public static ChatLiveFragment newInstance(int status, boolean isquestion) {
        ChatLiveFragment chatFragment = new ChatLiveFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        return inflater.inflate(R.layout.chat_live_fragment, null);
    }
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        tv_hide = (TextView) getView().findViewById(R.id.tv_hide);
        rlFragment = (RelativeLayout) getView().findViewById(R.id.rl_fragment);
    ...
    public void performSend(String content, int chatEvent) {
        switch (status) {
            ...
            case VhallUtil.WATCH_LIVE://观看直播界面发聊天和问答
                if (chatEvent == ChatLiveFragment.CHAT_EVENT_CHAT) {
                    mPresenter.sendChat(content);
                } else if (chatEvent == ChatLiveFragment.CHAT_EVENT_QUESTION) {
...
//                    Glide.with(getActivity()).load(data.avatar).placeholder(R.drawable.icon_vhall).into(viewHolder.iv_chat_avatar);
                    Glide.with(getActivity()).load(PrefUtils.getString(getActivity(), Constants.USER_IMG,"")).placeholder(R.drawable.icon_vhall).into(viewHolder.iv_chat_avatar);
                    switch (data.event) {
                        case ChatServer.eventMsgKey:
                            String[] kk = data.msgData.text.split("::",0);
                            if (kk.length>=2){
                                viewHolder.tv_chat_content.setVisibility(View.VISIBLE);
                                viewHolder.tv_chat_content.setText(EmojiUtils.getEmojiText(mActivity, kk[1]), TextView.BufferType.SPANNABLE);
                                viewHolder.tv_chat_name.setText(kk[0]);
                            }else {
                            viewHolder.tv_chat_content.setVisibility(View.VISIBLE);
    ...
    class QuestionAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            Holder viewHolder;
            if (convertView == null) {
                convertView = View.inflate(getActivity(), R.layout.chat_live_question_item, null);
