...
//        VhallSDK.getInstance().initWatch(param.watchId, param.userName, param.userCustomId, param.userVhallId, param.key, getWatchPlayback(), new VhallSDK.RequestCallback() {
        VhallSDK.getInstance().initWatch(param.watchId, param.userName, " ", param.userVhallId, param.key, getWatchPlayback(), new VhallSDK.RequestCallback() {
            @Override
    ...
    public WatchPlayback getWatchPlayback() {
        if (watchPlayback == null) {
            WatchPlayback.Builder builder = new WatchPlayback.Builder().context(watchView.getActivity()).containerLayout(playbackView.getContainer()).callback(new WatchCallback());
    ...
    public void sendChat(final String text) {
        if (TextUtils.isEmpty(param.userVhallId)) {
            Toast.makeText(watchView.getActivity(), com.vhall.uilibs.R.string.vhall_login_first, Toast.LENGTH_SHORT).show();
            return;
        }
